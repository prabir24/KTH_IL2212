...
echo -e "\n"
echo "*********************************************************"
echo "Press 1 for PERFORMANCE MODE"
echo "Press 2 for DEBUG MODE"
echo "*********************************************************"
echo -e "\n"

logFlag=0

read -p "Enter one of the above option : " userInput

if ([ $userInput != 1 ] && [ $userInput != 2 ]) || [ -z "$userInput" ]
then
	echo "WRONG INPUT.. PLEASE TRY AGAIN FROM ABOVE OPTIONS..."
	exit 1 
fi

if [ $userInput == 1 ]
then
	DEFINED_SYMBOL=PERFORMANCE
elif [ $userInput == 2 ]
then 
	DEFINED_SYMBOL=DEBUG
fi
...
nios2-app-generate-makefile \
    ...
    --set APP_CFLAGS_DEFINED_SYMBOLS -D ${DEFINED_SYMBOL}
